...
router.get('/all', (req, res) => {
  ...
  exec(`pm2 logs tiktok-downloader --lines ${lines} --nostream 2>&1`, (err, stdout) => {
    logs.pm2['tiktok-downloader'] = err ? `Error: ${err.message}` : (stdout || 'No logs');
    checkComplete();
  });
...
router.get('/pm2', (req, res) => {
  ...
  const allowed = ['rashmaliarefan', 'tiktok-downloader'];
